...
11. NEVER use multiple HTTPRequestsNodes connected to the ServerNode. Use exactly ONE.
12. Use `ClientJSNode` for frontend browser Javascript (DOM, events), NOT `JSNode`.
13. ALWAYS provide filenames with extensions for styling and scripts, e.g. `"css_filename": "style.css"` and `"filename": "script.js"`.
14. NEVER nest CSS or JS inside a `"seed"` object in the `RenderNode` config. Output them as separate CSSNode and ClientJSNode correctly linked to the RenderNode.
